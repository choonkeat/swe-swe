...
# When the docker daemon is on a different host (sibling-container
# setup), the relative ./home volume in the base compose resolves to
# our path, not theirs. Override with absolute host paths.
if [ "${SIBLING_DOCKER}" = "1" ]; then
    cat > "${PROJECT_PATH}docker-compose.override.yml" <<EOF
# Auto-generated by tunnel-up-manual.sh: rewrite bind mounts to host
# paths so the docker daemon can find them.
services:
  swe-swe:
    volumes:
