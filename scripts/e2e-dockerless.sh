...
HOMEPAGE="$(curl -s "http://127.0.0.1:$E2E_PORT/")"
if echo "$HOMEPAGE" | grep -q '<title>swe-swe</title>'; then
    pass "homepage is swe-swe"
else
    fail "homepage title wrong; body head: $(echo "$HOMEPAGE" | head -c 200 | tr '\n' ' ')"
fi
