...
test: ascii-check check-gomod-sync check-prctx-sync check-mcp-cli-proxy-sync check-mcp-sync check-swe-run-sync test-cli test-js test-www test-mcp-lazy-init test-mcp-cli-proxy test-mcp test-server test-git-sign-swe-swe test-swe-npx test-prctx test-swe-run
...
# The setup page (www/index.html) drives every answer through real DOM, so it
# needs a browser. Skips itself -- it does not fail -- when Playwright or a
# launchable chromium is missing, so this stays safe in the default chain.
test-www:
	node --test www/setup-page.test.mjs

test-mcp-lazy-init:
	go test -v ./cmd/mcp-lazy-init

# mcp-cli-proxy canonical source lives in cmd/mcp-cli-proxy (with tests); a
# non-test copy is bundled into the container image via
# cmd/swe-swe/templates/host/mcp-cli-proxy.
test-mcp-cli-proxy:
	go test -v ./cmd/mcp-cli-proxy

# Guard against the bundled mcp-cli-proxy copy drifting from the tested one.
check-mcp-cli-proxy-sync:
	@for f in cmd/mcp-cli-proxy/*.go; do \
		case "$$f" in *_test.go) continue;; esac; \
		t="cmd/swe-swe/templates/host/mcp-cli-proxy/$$(basename $$f)"; \
		if ! diff -q "$$f" "$$t" >/dev/null 2>&1; then \
			echo "mcp-cli-proxy bundle out of sync: $$f != $$t"; \
			echo "run: make sync-mcp-cli-proxy"; \
			diff "$$f" "$$t" || true; \
			exit 1; \
		fi; \
	done
	@echo "mcp-cli-proxy bundle in sync."

# Regenerate the bundled mcp-cli-proxy copy from the canonical source.
sync-mcp-cli-proxy:
	@rm -f cmd/swe-swe/templates/host/mcp-cli-proxy/*.go
	@for f in cmd/mcp-cli-proxy/*.go; do \
		case "$$f" in *_test.go) continue;; esac; \
...
# Guard against the bundled prctx copy drifting from the tested canonical one.
# Compares every non-test .go file; fails with a diff if they differ.
check-prctx-sync:
	@for f in cmd/prctx/*.go; do \
		case "$$f" in *_test.go) continue;; esac; \
		t="cmd/swe-swe/templates/host/prctx/$$(basename $$f)"; \
		if ! diff -q "$$f" "$$t" >/dev/null 2>&1; then \
			echo "prctx bundle out of sync: $$f != $$t"; \
			echo "run: make sync-prctx"; \
			diff "$$f" "$$t" || true; \
			exit 1; \
		fi; \
	done
	@echo "prctx bundle in sync."

# Regenerate the bundled prctx copy from the canonical cmd/prctx source.
sync-prctx:
	@rm -f cmd/swe-swe/templates/host/prctx/*.go
	@for f in cmd/prctx/*.go; do \
		case "$$f" in *_test.go) continue;; esac; \
		cp "$$f" cmd/swe-swe/templates/host/prctx/; \
	done
...
test-e2e-agent-view-remote-tunnel:
	E2E_AV_TUNNEL=1 ./scripts/e2e-agent-view-remote.sh

test-e2e-agent-view-remote-tunnel-image:
...
golden-update: build-cli
	...
	@$(MAKE) _golden-variant NAME=pi-only FLAGS="--agents pi"
	...
	@$(MAKE) _golden-variant NAME=runtime-container FLAGS="--runtime=container"
	...
	@$(MAKE) _golden-variant NAME=single-port-ssl FLAGS="--single-port --ssl selfsign"
