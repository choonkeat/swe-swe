...
All phases implemented and verified. Docker build with enterprise certificates now succeeds.
...
### What Was Fixed
- ✅ Docker build no longer fails with "SSL certificate problem: self-signed certificate in certificate chain" when enterprise certificates are present
- ✅ Node.js installation via NodeSource setup script now succeeds with enterprise certificates
- ✅ Certificates detected during `swe-swe init` are automatically included in generated Dockerfile
- ✅ Generated Dockerfiles include CERTS block only when certificates are actually present (no overhead when not needed)

### Commits Created
1. **Phase 1**: `refactor: add hasCerts parameter to Dockerfile template processing`
   - Added `hasCerts` bool parameter to `processDockerfileTemplate()` function
   - Updated `handleCertificates()` to return bool instead of void
   - Added test variants for certificate scenarios (baseline - no functional change yet)

2. **Phase 2**: `feat: add CERTS block to Dockerfile template for enterprise certificate support`
   - Added `{{IF CERTS}}...{{ENDIF}}` block before Node.js installation
   - Block copies certs and installs to system trust store before curl commands run
   - Golden files updated (still showing no CERTS block since `hasCerts=false` was passed)

3. **Phase 3**: `fix: reorder certificate detection before Dockerfile template processing to enable conditional CERTS block`
   - Moved `handleCertificates()` call to run BEFORE Dockerfile processing
   - Wired up `hasCerts` boolean return value to template processor
   - Golden files show CERTS block now appearing in cert-specific variants
   - Fixed Makefile to unset cert env vars in non-cert golden variants (ensures deterministic behavior)

4. **Phase 4a**: `fix: ensure non-cert golden variants unset certificate environment variables`
   - Updated Makefile targets to explicitly unset NODE_EXTRA_CA_CERTS, SSL_CERT_FILE, NODE_EXTRA_CA_CERTS_BUNDLE
   - Prevents host machine's cert env vars from leaking into test variants
   - Golden files regenerated with env var cleanup

5. **Phase 4b**: `fix: remove certs from .dockerignore to support certificate installation during Docker build`
   - Removed `certs/` exclusion from .dockerignore template
   - Certs are now included in Docker build context (needed for COPY during build)
   - Note added explaining why certs are not excluded
   - Golden files updated

### Verification Results
- ✅ All unit tests pass (TestGoldenFiles, TestProcessDockerfileTemplate, etc.)
- ✅ Golden files show correct behavior:
  - `with-certs-node-extra-ca-certs`: Includes CERTS block in Dockerfile
  - `with-certs-ssl-cert-file`: Includes CERTS block in Dockerfile
  - `with-certs-no-certs`: Does NOT include CERTS block (correct)
  - `default` and other non-cert variants: Do NOT include CERTS block (correct)
- ✅ End-to-end Docker build test:
  - Project: tiny-form-fields with NODE_EXTRA_CA_CERTS env var set
  - Init: `./dist/swe-swe.darwin-arm64 init --project-directory=/Users/choonkeatchew/git/choonkeat/tiny-form-fields --previous-init-flags=reuse`
  - Build: `docker-compose build swe-swe` from .swe-swe project directory
  - Result: ✅ Image built successfully with Node.js installation completing without SSL errors
  - Final image: `users-choonkeatchew-git-choonkeat-tiny-form-fields-90e65a32-swe-swe:latest`

### Known Limitations / Future Considerations
1. Certificate environment variables checked: NODE_EXTRA_CA_CERTS, SSL_CERT_FILE, NODE_EXTRA_CA_CERTS_BUNDLE
   - Could extend to check NODE_EXTRA_CA_CERTS_BUNDLE and SSL_CERT_DIR in future
   - Current implementation covers primary use cases
2. Dockerfile function signature is positional-parameter heavy
   - User noted this could be error-prone
   - Could be refactored to use struct/options pattern in future
3. Both certs and .env/.path files are now in Docker build context
   - Minimal impact (very small files)
   - Could be optimized by filtering build context if needed

## Rollback Plan

If issues arise:
1. Revert "fix: remove certs from .dockerignore..." commit (last)
2. Revert "fix: ensure non-cert golden variants unset..." commit
3. Revert "fix: reorder certificate detection..." commit (Phase 3)
4. Revert "feat: add CERTS block..." commit (Phase 2)
5. Revert "refactor: add hasCerts parameter..." commit (Phase 1)
6. Project returns to original state with original behavior
