...
- **"Commit the log, then end" now runs your command, not ours**: The button used to send an instruction baked into the binary -- run `/swe-swe:commit-session-chat-log`, then `end_session` with this uuid -- so what the button did was ours to decide, not yours. The server now looks for a flat `commit-log-then-end` command in the session agent's own command directory (the same lookup that builds the `/` autocomplete list: project directory first, then system directory) and, when one exists, sends only `/commit-log-then-end`. Finding the file in *that* agent's directory is what proves the agent can run it, so agents with no slash-command convention (aider, shell) are never handed a name they would type out as literal text. Nothing is bundled under `swe-swe:` on purpose: a bundled copy would always be found, the fallback advice would never appear, and `init` wipes and re-seeds that namespace on every upgrade, so an edited copy would not survive. Alongside this, `end_session`'s `uuid` argument is now optional and defaults to the calling session, derived from its per-session auth key -- an agent ending itself no longer has to name (or mistype) a UUID, and a hand-written command carries no stale id that could end someone else's session. Explicit uuids still work.

- **Session ID is copyable, and chat file links open in the Files pane**: Session Settings > Profile gains a read-only Session ID field with a Copy button, reusing the copy-with-feedback control already used by the Share link and the password. Separately, the chat iframe's src now carries `files_url` (this session's file-browser origin) whenever the files proxy port is known, which switches on agent-chat's workspace-file links: cmd-click opens the file in a real browser tab, and a plain click posts `agent-chat-open-files` back to the parent frame, which reveals the file in the Files pane -- mirroring the existing Preview reveal. The URL is rebuilt on the parent side from the workspace-relative path rather than trusted from the iframe, so a `files_url` that went stale (port reassigned after the src was built) cannot point the pane at a dead origin. With no `files_url`, agent-chat keeps its previous link behaviour.

### Fixes

- **iPhone: the app no longer collapses to a strip when the keyboard opens, and Session Settings' footer stays reachable**: Two related layout bugs with one root cause -- on mobile Safari the layout viewport, `100%` heights and every `vh` unit can hold a stale height, and no resize event corrects them after a tab is backgrounded and restored or while the toolbar or keyboard is up. Symptom one: the app painted short, leaving a dead band between its bottom edge and the Safari toolbar, and Session Settings was clipped at that same stale edge so its footer -- the row holding End session -- landed outside the drawn area with nothing to scroll; reloading the page was the only way to reach it. `syncVisibleViewport()` now publishes `window.visualViewport` (the band actually on screen) to the two places that trusted the layout viewport: an `--app-viewport-height` custom property on `html`/`body`, and inline `top`/`height` on the fixed `.settings-panel`. It runs on visualViewport resize/scroll and on every path back into the page (`pageshow`, `focus`, `visibilitychange`, `orientationchange`) plus when the panel opens; the panel's max-height caps became percentages of that band instead of `vh`, the panel body got `overflow: hidden` with `min-height: 0` on the pane host so an unshrinkable pane scrolls rather than pushing the footer past the content box's clipped edge, and mobile gained `padding-bottom: env(safe-area-inset-bottom)` to clear the home indicator. Symptom two, found after that landed: tapping the message box collapsed the whole app into a strip a couple of hundred pixels tall at the top of the screen, with only the input row on screen and the header and conversation above the visible band. When the keyboard opens over a document that cannot scroll, mobile Safari scrolls the *visual* viewport rather than the document (`visualViewport.offsetTop > 0` while `window.scrollY` stays 0) -- the app had been told how tall the visible band was but not where it was, so it stayed at the document origin while the band slid down past it, and the dead space measured exactly `offsetTop`. `visualViewport.offsetTop` is now published as `--app-viewport-offset` and the terminal-ui element is `position: fixed` against both properties; fixed elements lay out against the layout viewport, which that scroll does not move, so `top: offsetTop` lands the app exactly on the visible band, and the `0px`/`100%` fallbacks reproduce the old static layout. The keyboard margin in `updateViewport()` is gone with it -- it pushed the bottom-most fixed element up by the keyboard height, correct when `html`/`body` filled the layout viewport but a second subtraction now that they are sized to the band above the keyboard. Verified on the reporting device and against a phone-sized harness built from the real markup and CSS. Known remaining issue, tracked in the agent-chat repo (`tasks/2026-07-31-keyboard-hides-last-messages.md`): the chat pane's own sticky input bar still covers the last few lines of the conversation when the keyboard opens -- a scroll-position bug inside agent-chat, not in this layout.

- **The stop guard missed real silent turns and passed fake sends**: The `PreToolUse`-style guard that blocks an agent-chat turn from ending without a user-visible message had two defects. (1) *Turn boundary*: the transcript slice anchored only on non-`tool_result` `type: user` lines, but an agent-chat reply arrives as a `tool_result` -- so back-to-back chat replies shared one anchor and an earlier `send_message` counted as covering the newer turn, letting it end silently. An anchor is now a typed user line **or** a `tool_result` carrying `User said:`/`User responded:`. (2) *Send detection*: a plain-text scan over the raw slice counted any line that merely named a send tool, including tool output that printed the name -- so a turn could pass on the strength of a string it never called. The slice is now parsed as structured records: only a real `tool_use` counts, and the command-line path is honoured only when the send command sits at the start of a Bash command rather than as an argument to another one. Covered by `cmd/swe-swe/hookstopguard_test.go` (16 cases: both fixes, the sibling progress/draw/verbal send tools, the empty-queue silent turn, the one-nudge-per-turn escape, and malformed transcript lines).

## v2.34.2 - Ending-Session Label

### Fixes

- **A session being torn down shows `Join Ending Session`, in red**: The card previously read `Join Session (Ending)`, where the parenthetical reads as an aside rather than as the state of the button you are about to press. The button now also carries the same muted red as the end dialog's `Discard the log and end` option, so a session on its way out looks the same wherever it is shown.
...
- **Stop/Cancel from Agent Chat now sends one Esc, not two**: The `agent-chat-interrupt` handler wrote `\x1b` twice back-to-back before typing its nudge, which was wrong in two independent ways. The two bytes arrive at the agent's input parser as the pair `ESC ESC`, and that parser reads ESC-then-byte as a meta sequence -- the same convention that makes `ESC` + `b` mean Alt+Left in our own key map -- so the pair collapsed into a single Alt+Esc event that is typically discarded, and the interrupt intermittently did nothing at all. Beyond that, the second Esc has no state in which it helps: the first already interrupts a running turn, dismisses a permission prompt, closes an `@`/`/` autocomplete popup, or clears a draft line, and the second merely advances one state further -- which from an idle prompt means Claude Code's Esc-Esc binding opens the "jump to a previous message" rewind picker, where the nudge text we type next is swallowed as filter input and the trailing Enter can select a rewind target. Now a single Esc, then the existing 300ms wait before the text and Enter. Vim-mode input remains the one unhandled case (Esc leaves insert mode, so the typed nudge is read as vim commands); it is not detectable from the parent frame.
...
- **"Commit the log, then end" is now the default, and every disposition lands on the homepage**: The option users almost always want was listed last (homepage) or second (settings panel) among three equal-looking cards. It now leads in both places, carries the accent tint plus a `Recommended` badge, and the irreversible Discard option sits last; the tint is deliberate rather than a filled button, since filling one of three equal-weight cards makes the other two read as disabled. The settings-panel flow also used to keep the user on the terminal behind an alert explaining that the agent would end the session itself -- redundant, since the homepage card already carries that state -- so all four dispositions now redirect to `/`.

### Fixes

- **Agent View recovers after a browser-backend restart or outage, in three more places**: (1) *The backend survives its own container restart*: `docker stop` + `docker start` preserves the container filesystem, so `/tmp/.X<n>-lock` and the `/tmp/.X11-unix` sockets outlived the Xvfb processes that owned them -- Xvfb then refused the display and exited 1, chromium died with it, and allocation still reported a healthy session, so every CDP connect failed with `connection refused` and Agent View was dead with no visible error. The entrypoint now clears stale X locks, X11 sockets and chromium session dirs on boot, and allocation fails loudly (`Xvfb on display :11 exited immediately ... stale X lock or display in use`) instead of advertising a session whose CDP port never answers. (2) *A failed initial allocation is no longer latched forever*: a session whose first browser tool call caught the backend down set `initDone = true` anyway, so every later call skipped init and never retried -- one observed session was browser-less for 9 hours after the backend came back. `mcp-lazy-init` now latches only on success, and retries transient failures (transport errors, 5xx) 4 times with 1s/2s/4s backoff so a merely-restarting backend is absorbed with no user-visible failure; 4xx is not retried. (3) *`/reboot-light` no longer kills the backend*: its stray-container kill list was "everything except self and tunneld", and `--restart=always` does not re-apply after an explicit `docker kill`, so the standalone browser backend stayed `Exited(137)` until someone ran `docker start` by hand.

- **Update badge: unreachable tooltip, and prereleases comparing equal**: The tooltip sat at `top: calc(100% + 8px)` and showed only on `.update-badge-wrap:hover`, making the 8px offset a dead zone -- the pointer left the badge, `:hover` dropped, and the tooltip vanished before its release-notes link could be entered; there was no pointer path to the link at all. An `.update-tip::before` strip now spans the offset from inside the tooltip, so hit-testing is continuous and the spacing is visually unchanged (verified by driving a real browser with the pointer parked at the gap midpoint). Separately, `isNewer()` stripped any `-rc1` suffix before comparing, so `2.34.1-rc1` read as equal to the `2.34.1` that superseded it and a prerelease box was never told the release had shipped; replaced with real semver precedence (semver.org rule 11.4) in a unit-tested `static/modules/semver.js`.

- **Codex sessions: chat-log export and event log were silently dropped**: Codex sandboxes MCP child processes and forwards only the env vars named in `env_vars`, and the `swe-swe-agent-chat` entry listed `AGENT_CHAT_PORT` but not `AGENT_CHAT_EVENT_LOG` or `AGENT_CHAT_EXPORT_DIR` -- so `chatlog_close` failed with `AGENT_CHAT_EXPORT_DIR is unset` and no event log was ever written. swe-swe did set both on the session process; the whitelist dropped them one level down. Other agents were unaffected (their configs wrap the command in `sh -c` and inherit the full env).

- **The branch dropdown lists plain branch names, never `origin/*`**: The New Session dialog offered remote-tracking duplicates (`origin/main` alongside `main`), which are not branches you can commit to.

- **No more iOS zoom when tapping a field**: Inputs, dropdowns and the Where/branch combo-box in the New Session dialog, plus the session settings panel and terminal UI inputs, were below the 16px threshold Mobile Safari uses to decide whether to zoom the page in on focus.

## v2.33.0 - End-Session Chat-Log Flow & New Session Dialog

### Features

- **Ending a session asks what should happen to its chat log**: With `agent-chats/` archiving on by default, ending a session left an uncommitted transcript behind with no prompt. The End button -- both the homepage card and the in-session Settings panel, which previously had no chat-log path at all -- now opens a dialog offering four dispositions: commit the log and end, keep it uncommitted, discard it, or cancel.

- **Sessions end asynchronously, and joins are refused mid-teardown**: `end_session` returns immediately and teardown proceeds in the background, so committing a large chat log no longer blocks the request; a session in the ending state refuses new joins rather than handing back a pane that is about to vanish.

- **`reboot_server` MCP tool and a self-healing shutdown page**: An agent can restart its own server (useful after an upgrade or a config change), and the page served during shutdown reconnects on its own once the server is back instead of leaving a dead tab.

- **New Session dialog: fewer required decisions**: Agent Chat is now the default channel and the Agent Terminal option is hidden; a sole available agent is auto-selected instead of demanding a pick; the required fields are no longer buried below optional ones; the dev-channels checkbox is gone and the Extra CLI flags box starts blank (it used to arrive prefilled with a flag most users did not want).

- **`prctx --token-env NAME`**: The PR/MR review helper can read its access token from any named environment variable, instead of only the one hard-coded name.
