...
## v2.34.2 - Chat-Log Archiving, Agent View Tunnel, swe-npx, Procfile Runner & End-Session Flow
...
- **Ending a session now asks what to happen to its chat log**: With `agent-chats/` archiving on by default, ending a session left an uncommitted transcript behind with no prompt. The End button -- both the homepage card and the in-session Settings panel, which previously had no chat-log path at all -- now opens a dialog offering four dispositions: commit the log and end, keep it uncommitted, discard it, or cancel. **"Commit the log, then end" leads and is badged `Recommended`** (it was listed last among three equal-looking cards), with the irreversible Discard option last; the styling is an accent tint rather than a filled button, since filling one of three equal-weight cards makes the other two read as disabled. Every disposition redirects to the homepage instead of leaving the user on a terminal behind an alert -- the homepage card already carries the state, and a session on its way out shows a red **`Join Ending Session`** button (previously the parenthetical `Join Session (Ending)`) in the same muted red as the dialog's Discard option.
