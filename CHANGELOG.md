...
## v2.24.0 - Session Fork, SSH Commit Signing, Skills from Git Repos & Tunnel mTLS

### Features

- **Session fork (`/api/fork` + `swe-swe-fork-convo` CLI)**: Fork a running or ended agent session into a new one that replays the same chat history and resumes the agent's memory. `/api/fork/<uuid>?bubble=<seq>&mode=after` anchors the fork at a specific chat bubble (falling back to the last agent reply when no `bubble` is given), prepopulates the new session's chat log so the forked tab shows identical prefix history, and 302-redirects with the agent's `--resume` wired in. Works for Claude and Codex (other agents rejected loudly; Pi deferred to its native fork). The source agent session id is now captured at spawn -- `--session-id` injection for Claude, a spawn-mutex + session-dir watch for Codex/Pi -- so forking stops guessing which `.jsonl` to fork; ended sessions hydrate from disk metadata, with a five-needle event-text fingerprint that recovers the id for legacy recordings. To avoid a blank first render, the fork is staged and materialized by the first WebSocket client so the PTY starts with real terminal geometry. A shared `forkconvo` package backs both the server endpoint and a standalone CLI
