...
- **execute-in-worktree wires agent chat + chat-export wrap-up**: The spawned worktree session now always passes `--channels server:agent-chat` (so chat is wired to the UI, not merely tool-present) and is directed to use `send_message`/`send_progress` rather than the unwatched TUI. On completion it exports the chat to `./agent-chats/` via `export_chat_md` and commits it alongside the work. The command also commits the task file before spawning, since a fresh worktree only contains committed content
