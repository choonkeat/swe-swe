...
func handlePassthrough(command string, args []string) {
	...
	for _, envVar := range os.Environ() {
		if strings.HasPrefix(envVar, "NODE_EXTRA_CA_CERTS=") ||
			...
			strings.HasPrefix(envVar, "SWE_SWE_AUTO_UPGRADE=") {
			continue
		}
...
// checkAndUpgrade checks if the CLI version differs from the config version
// and handles the upgrade flow. Returns the (possibly modified) remainingArgs.
func checkAndUpgrade(sweDir, absPath string, remainingArgs []string) []string {
	config, err := loadInitConfig(sweDir)
	if err != nil {
	...
	autoUpgrade := os.Getenv("SWE_SWE_AUTO_UPGRADE") != ""
	if !autoUpgrade && !isInteractiveTerminal() {
		fmt.Fprintf(os.Stderr, "Warning: swe-swe CLI updated (%s -> %s) but container config is stale.\n", config.CLIVersion, Version)
		fmt.Fprintf(os.Stderr, "Run 'swe-swe init --previous-init-flags=reuse' then 'swe-swe up --build' to upgrade.\n\n")
		return remainingArgs
	}

	fmt.Printf("swe-swe CLI updated (%s -> %s).\n", config.CLIVersion, Version)
	doUpgrade := autoUpgrade
	if !doUpgrade {
		fmt.Printf("Container config was generated by the older version.\nUpgrade? [y/N] ")
		reader := bufio.NewReader(os.Stdin)
		answer, _ := reader.ReadString('\n')
		answer = strings.TrimSpace(strings.ToLower(answer))
		doUpgrade = answer == "y" || answer == "yes"
	}
	if !doUpgrade {
		return remainingArgs
	}

	fmt.Printf("\nRegenerating config with saved flags...\n")
	initCmd := exec.Command(os.Args[0], "init",
		"--previous-init-flags=reuse",
	...
	// Inject --build if not already present
	hasBuild := false
	...
	if !hasBuild {
		remainingArgs = append(remainingArgs, "--build")
	}
	fmt.Printf("\nStarting with --build to rebuild container...\n\n")
	return remainingArgs
}
