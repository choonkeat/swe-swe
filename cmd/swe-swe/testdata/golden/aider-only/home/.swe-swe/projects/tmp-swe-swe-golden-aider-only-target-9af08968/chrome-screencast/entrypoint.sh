...
# Chrome Screencast Container Entrypoint
# This script:
# 1. Installs enterprise certificates into:
...
if [ -d /swe-swe/certs ] && [ "$(find /swe-swe/certs -type f -name '*.pem' 2>/dev/null | wc -l)" -gt 0 ]; then
    ...
    # Set ownership for pwuser
    chown -R pwuser:pwuser "$NSS_DB"
