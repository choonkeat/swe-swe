var CanvasBundle = (() => {
  ...
  // canvas-entry.ts
  var canvas_entry_exports = {};
  __export(canvas_entry_exports, {
    AgentWhiteboard: () => AgentWhiteboard,
  ...
  function bezierLength(x0, y0, x1, y1, x2, y2, x3, y3, steps = 20) {
    let len = 0;
    let prevX = x0;
    let prevY = y0;
    for (let i2 = 1; i2 <= steps; i2++) {
      const t2 = i2 / steps;
      const cx = bezierPoint(x0, x1, x2, x3, t2);
      const cy = bezierPoint(y0, y1, y2, y3, t2);
  ...
  function subdivideBezier(x0, y0, x1, y1, x2, y2, x3, y3, t2) {
    const ax = lerp(x0, x1, t2);
    const ay = lerp(y0, y1, t2);
    const bx = lerp(x1, x2, t2);
    const by = lerp(y1, y2, t2);
    const cx = lerp(x2, x3, t2);
    const cy = lerp(y2, y3, t2);
    const dx = lerp(ax, bx, t2);
    const dy = lerp(ay, by, t2);
    const ex = lerp(bx, cx, t2);
    const ey = lerp(by, cy, t2);
    const fx = lerp(dx, ex, t2);
    const fy = lerp(dy, ey, t2);
    return [x0, y0, ax, ay, dx, dy, fx, fy];
  }
  function measureOps(ops) {
    const segments = [];
    let curX = 0;
    let curY = 0;
    let cumLen = 0;
  ...
  var ProgressiveAnimator = class _ProgressiveAnimator {
     ...
     */
    static animate(ctx, ops, options) {
      const { duration, strokeColor, strokeWidth, easing = easeOutCubic } = options;
      const segments = measureOps(ops);
      const totalLength = segments.length > 0 ? segments[segments.length - 1].cumulativeLength : 0;
      if (totalLength === 0 || duration <= 0) {
        _ProgressiveAnimator.drawOpsToLength(ctx, segments, totalLength, strokeColor, strokeWidth);
      ...
      return new Promise((resolve) => {
        ...
        const tick = (now) => {
          ...
          _ProgressiveAnimator.drawOpsToLength(ctx, segments, targetLength, strokeColor, strokeWidth);
          if (rawProgress < 1) {
     ...
     */
    static drawOpsToLength(ctx, segments, targetLength, strokeColor, strokeWidth) {
      ctx.save();
      ...
      for (let i2 = 0; i2 < segments.length; i2++) {
        ...
        const prevCum = i2 > 0 ? segments[i2 - 1].cumulativeLength : 0;
  ...
  var TextRenderer = class _TextRenderer {
    static animate(ctx, text, x2, y2, options) {
      ...
      if (charCount === 0 || duration <= 0) {
        _TextRenderer.drawText(ctx, text, x2, y2, font, fontSize, color);
        return Promise.resolve();
      }
      return new Promise((resolve) => {
        const startTime = performance.now();
        const tick = (now) => {
          ...
          _TextRenderer.drawText(ctx, partial, x2, y2, font, fontSize, color);
          if (progress < 1) {
            requestAnimationFrame(tick);
          } else {
            resolve();
    ...
    static drawText(ctx, text, x2, y2, font, fontSize, color) {
      ctx.save();
      ...
      const bgX = x2 - padding;
      const bgY = y2 - textHeight / 2 - padding;
  ...
  var RoughRenderer = class {
    ...
    resize(logicalW, logicalH, dpr = 1) {
      this.dpr = dpr;
      ...
      this.rc = at.canvas(this.displayCanvas);
      if (typeof OffscreenCanvas !== "undefined") {
      ...
      this.persistCtx.drawImage(oldPersist, 0, 0);
    ...
    async animateLine(x1, y1, x2, y2, color, width) {
    ...
    /** Animate a rectangle */
    async animateRect(x2, y2, w2, h2, color, width, fill, fillStyle) {
      const strokeColor = color ?? this.options.strokeColor;
      const strokeWidth = width ?? this.options.strokeWidth;
      const opts = {
        roughness: this.options.roughness,
      ...
      const drawable = this.rc.generator.rectangle(x2, y2, w2, h2, opts);
      await this.animateDrawable(drawable, strokeColor, strokeWidth);
      this.commitDrawable(drawable, strokeColor, strokeWidth, fill);
    }
    /** Animate a circle */
    async animateCircle(x2, y2, radius, color, width, fill, fillStyle) {
      const strokeColor = color ?? this.options.strokeColor;
      const strokeWidth = width ?? this.options.strokeWidth;
      ...
      const drawable = this.rc.generator.circle(x2, y2, diameter, opts);
      await this.animateDrawable(drawable, strokeColor, strokeWidth);
      this.commitDrawable(drawable, strokeColor, strokeWidth, fill);
    }
    /** Animate an ellipse */
    async animateEllipse(x2, y2, w2, h2, color, width, fill, fillStyle) {
      const strokeColor = color ?? this.options.strokeColor;
      const strokeWidth = width ?? this.options.strokeWidth;
      const opts = {
        roughness: this.options.roughness,
      ...
      const drawable = this.rc.generator.ellipse(x2, y2, w2, h2, opts);
      await this.animateDrawable(drawable, strokeColor, strokeWidth);
      this.commitDrawable(drawable, strokeColor, strokeWidth, fill);
    }
    /** Animate text with typewriter effect */
    async animateText(text, x2, y2, options) {
      ...
      const dur = this.skipAnimation ? 0 : this.duration(text.length * 50);
      await this.animateWithOverlay(
        (ctx) => TextRenderer.animate(ctx, text, x2, y2, { duration: dur, font, fontSize, color })
      );
    ...
    /** Enable or disable skip-animation mode (instant drawing, no rAF loops). */
    ...
    compositeToDisplay() {
      ...
      this.displayCtx.save();
      this.displayCtx.setTransform(1, 0, 0, 1, 0, 0);
      this.displayCtx.drawImage(this.persistCanvas, 0, 0);
      this.displayCtx.restore();
    }
    /**
     ...
     */
    async animateDrawable(drawable, strokeColor, strokeWidth) {
      ...
      const baseDuration = Math.min(2e3, Math.max(200, totalLength * 2));
  ...
  var AgentWhiteboard = class {
    ...
    /** Enable or disable skip-animation mode (instant drawing). */
    setSkipAnimation(skip) {
      this._skipAnimation = skip;
      this.renderer.setSkipAnimation(skip);
    }
    /** Clear the canvas (keeps drawing state). */
    clear() {
      this.renderer.clear();
    }
    /** Full reset: clear canvas, reset drawing state, clear pending queue. */
    reset() {
      this.queue.reset();
      this.renderer.clear();
      this.curX = this.logicalWidth / 2;
      this.curY = this.logicalHeight / 2;
      this.color = "#000000";
      this.strokeWidth = 2;
    }
    ...
    async executeInstruction(instruction, index) {
      this.events.onInstructionStart?.(instruction, index);
      switch (instruction.type) {
        case "moveTo":
          this.curX = instruction.x;
          this.curY = instruction.y;
          break;
        case "lineTo": {
        ...
        case "setColor":
          this.color = instruction.color;
          break;
        case "setStrokeWidth":
          this.strokeWidth = instruction.width;
          break;
        case "drawRect":
        ...
        case "label":
          await this.renderer.animateText(
            instruction.text,
            this.curX + (instruction.offsetX ?? 10),
            this.curY + (instruction.offsetY ?? -20),
            {
              fontSize: instruction.fontSize,
              color: this.color
            }
  ...
  function suggestType(input) {
    ...
    for (const t2 of VALID_TYPES) {
      if (t2.toLowerCase() === lower) return t2;
    }
    for (const t2 of VALID_TYPES) {
      const tLower = t2.toLowerCase();
      if (tLower.includes(lower) || lower.includes(tLower)) return t2;
    }
    if (lower.length >= 3) {
      for (const t2 of VALID_TYPES) {
        if (t2.toLowerCase().startsWith(lower.slice(0, 3))) return t2;
      }
    }
    return null;
  }
    ...
    for (let i2 = 0; i2 < raw.length; i2++) {
      ...
      if (item === null || typeof item !== "object" || Array.isArray(item)) {
        errors.push({
          index: i2,
          type: item,
          message: 'instruction is not an object. Each instruction must be a JSON object like {"type":"moveTo","x":100,"y":100}'
        });
        continue;
      }
      const obj = item;
      if (!("type" in obj) || typeof obj.type !== "string") {
        errors.push({
          index: i2,
          type: void 0,
          message: 'missing or non-string "type" field. Each instruction needs a "type" field, e.g. {"type":"drawRect","x":100,"y":100,"width":200,"height":80}'
        });
        continue;
      }
  ...
  function formatValidationErrors(errors, totalCount) {
    const lines = errors.map((e2) => {
      const typeLabel = e2.type !== void 0 ? ` (type "${e2.type}")` : "";
      return `  - Instruction #${e2.index}${typeLabel}: ${e2.message}`;
    });
    return `VALIDATION ERRORS (${errors.length} of ${totalCount} instructions were invalid and skipped):
${lines.join("\n")}`;
  }
  return __toCommonJS(canvas_entry_exports);
})();
