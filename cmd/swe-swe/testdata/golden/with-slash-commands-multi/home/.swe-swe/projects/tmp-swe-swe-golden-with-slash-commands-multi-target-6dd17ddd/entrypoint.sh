...
if [ -d "/home/app/.swe-swe/commands/md/org/team-cmds" ]; then
    mkdir -p "/home/app/.pi/agent/prompts"
    # Prune links from an earlier bundle before re-linking, so a command this
    # repo renamed or removed does not linger in pi's autocomplete.
    find "/home/app/.pi/agent/prompts" -maxdepth 1 -type l -name 'org/team-cmds-*.md' -delete 2>/dev/null || true
    # The pre-flat design linked the repo in as one directory, which pi never
    # read. Drop it so it cannot double every command if pi learns to recurse.
    if [ -L "/home/app/.pi/agent/prompts/org/team-cmds" ]; then rm -f "/home/app/.pi/agent/prompts/org/team-cmds"; fi
    for f in "/home/app/.swe-swe/commands/md/org/team-cmds"/*.md; do
        [ -e "$f" ] || continue
        piLink="/home/app/.pi/agent/prompts/org/team-cmds-$(basename "$f")"
        if [ -e "$piLink" ] && [ ! -L "$piLink" ]; then
            echo -e "${YELLOW}⚠ Slash command target exists and is not a symlink, leaving unchanged: $piLink (pi)${NC}"
        else
            ln -sfn "$f" "$piLink"
...
command = "swe-npx"
args = ["-y", "@choonkeat/agent-chat", "--theme-cookie", "swe-swe-theme", "--welcome-replies", "What can you help me with?,Give me an overview of this project,What has changed recently?,Discuss: which worktrees & branches can we clean up?,/swe-swe:recordings-list-orphaned", "--autocomplete-triggers", "/=slash-command", "--autocomplete-url", "http://localhost:$SWE_SERVER_PORT/api/autocomplete/$SESSION_UUID?key=$MCP_AUTH_KEY"]
...
claude_mcp_setup() {
  ...
  claude mcp add --scope user --transport stdio swe-swe-agent-chat -- sh -c 'exec swe-npx -y @choonkeat/agent-chat --theme-cookie swe-swe-theme --welcome-replies "What can you help me with?,Give me an overview of this project,What has changed recently?,Discuss: which worktrees & branches can we clean up?,/swe-swe:recordings-list-orphaned" --autocomplete-triggers /=slash-command --autocomplete-url http://localhost:$SWE_SERVER_PORT/api/autocomplete/$SESSION_UUID?key=$MCP_AUTH_KEY'
