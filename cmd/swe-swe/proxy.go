...
// proxyDir is the directory where the daemon writes the container script,
// PID file, and per-request files. Default is CWD-relative (project tier);
// `swe-swe proxy --global` switches it to `$HOME/.swe-swe/proxy` (global tier).
var proxyDir = ".swe-swe/proxy"

// defaultContainerProxyDir is the in-container path the generated container
// script falls back to when PROXY_DIR is unset in the container's environment.
//
// For the project tier (default), the script lives at /workspace/.swe-swe/proxy/<cmd>
// and the container's CWD is /workspace, so the relative path resolves correctly.
//
// For the global tier (--global), the script lives at $HOME/.swe-swe/proxy/<cmd>
// on the host, which is bind-mounted to /home/app/.swe-swe/proxy/<cmd> inside the
// container. The script may be invoked from any CWD, so we hardcode the absolute
// in-container path.
const (
	defaultContainerProxyDirProject = ".swe-swe/proxy"
	defaultContainerProxyDirGlobal  = "/home/app/.swe-swe/proxy"
)
...
//
// Format args (in order): command, command, defaultProxyDir.
...
// handleProxy implements the `swe-swe proxy [--global] <command>` subcommand.
...
func handleProxy() {
	...
	fs.Usage = printProxyHelp
	globalFlag := fs.Bool("global", false, "Write proxy files to $HOME/.swe-swe/proxy (global tier) instead of CWD-relative .swe-swe/proxy (project tier)")
	if err := fs.Parse(os.Args[2:]); err != nil {
		// flag.ContinueOnError prints its own error; we just exit.
		os.Exit(2)
	}
	...
	// --global switches the daemon's proxy directory from CWD-relative to
	// $HOME/.swe-swe/proxy. The container sees this dir at
	// /home/app/.swe-swe/proxy via a sub-mount layered on top of the home mount.
	defaultContainerProxyDir := defaultContainerProxyDirProject
	if *globalFlag {
		home, err := os.UserHomeDir()
		if err != nil || home == "" {
			fmt.Fprintf(os.Stderr, "Error: --global requires $HOME to be set: %v\n", err)
			os.Exit(1)
		}
		proxyDir = filepath.Join(home, ".swe-swe", "proxy")
		defaultContainerProxyDir = defaultContainerProxyDirGlobal
	}
	...
	if err := generateContainerScript(scriptFile, command, defaultContainerProxyDir); err != nil {
		os.Remove(pidFile)
...
// defaultProxyDir is baked into the script as the fallback PROXY_DIR when the
// container does not set the env var explicitly.
func generateContainerScript(path, command, defaultProxyDir string) error {
	script := fmt.Sprintf(containerScriptTemplate, command, command, defaultProxyDir)
...
func printProxyHelp() {
	fmt.Fprintf(os.Stderr, `Usage: swe-swe proxy [--global] <command>
...
Flags:
  --global     Write proxy files to $HOME/.swe-swe/proxy (global tier),
               visible to every project's container as /home/app/.swe-swe/proxy/<command>.
               Default is CWD-relative .swe-swe/proxy (project tier), visible only
               to the container of the project the daemon was started in.

Environment Variables:
  PROXY_TIMEOUT    Timeout in seconds for container script (default: 300)
  PROXY_DIR        Override proxy directory inside the container script (advanced)

How It Works:
  1. Host runs 'swe-swe proxy <command>' which watches the proxy dir for requests
  2. Container runs '<command> [args...]' (resolved on PATH) to submit a request
...
Tiers:
  project tier (default)   $PWD/.swe-swe/proxy  ->  /workspace/.swe-swe/proxy/<cmd>
                           per-project, lives in repo, can be team-shared via git
  global tier (--global)   $HOME/.swe-swe/proxy ->  /home/app/.swe-swe/proxy/<cmd>
                           per-host, applies to every project's container

PATH order inside the container is: project tier > global tier > stack shims >
system, so a project-defined proxy overrides a global one of the same name.

File Protocol (relative to the chosen tier's directory):
...
Examples:
  # Host: start project-tier proxy for 'make' from inside a project dir
  cd /repos/myapp
  swe-swe proxy make
  # Container: just run 'make build TARGET=hello' (PATH resolves to the script)

  # Host: start a global-tier proxy for 'gh' visible to every project
  swe-swe proxy --global gh

  # Multiple proxies (run in separate terminals)
  swe-swe proxy make
  swe-swe proxy --global gh
  swe-swe proxy --global docker
